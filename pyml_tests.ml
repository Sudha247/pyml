  ...
  Pyml_tests_common.add_test ~title:"numpy"
    (fun () ->
      if Py.Import.try_import_module "numpy" = None then
        ...
        begin
          let array = Stdlib.Array.Floatarray.create 2 in
          Stdlib.Array.Floatarray.set array 0 1.;
          Stdlib.Array.Floatarray.set array 1 2.;
...
array[1] = 43.
");
          assert (Stdlib.Array.Floatarray.get array 0 = 42.);
          assert (Stdlib.Array.Floatarray.get array 1 = 43.);
          Pyml_tests_common.Passed
        end)
  ...
  Pyml_tests_common.add_test ~title:"numpy crunch"
    (fun () ->
      if Py.Import.try_import_module "numpy" = None then
        ...
        begin
          let array = Stdlib.Float.Array.init 0x10000 float_of_int in
          ...
          let rec crunch numpy_array n =
            if n <= 0 then
              ...
              let array =
                Stdlib.Float.Array.map_from_array Stdlib.Fun.id
          ...
          assert (Stdlib.Float.Array.length array = 0x10000);
          for i = 0 to 0x10000 - 1 do
            assert (Stdlib.Float.Array.get array i = float_of_int i)
          done;
          Stdlib.Float.Array.set array 1 42.;
